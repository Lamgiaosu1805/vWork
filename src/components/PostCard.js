...
// ── AuthAvatar ────────────────────────────────────────────────────────────────
export const AuthAvatar = ({ filename, name, size = 40 }) => {
    const [uri, setUri] = useState(null);
    useEffect(() => {
        if (!filename) return;
        let cancelled = false;
    ...
    const sz = { width: size, height: size, borderRadius: size / 2, marginRight: 7 };
    if (uri) return <Image source={{ uri }} style={sz} resizeMode="cover" />;
    return (
        <View style={[s.avatarFallback, sz]}>
            <Text style={[s.avatarInitials, { fontSize: size * 0.38 }]}>{getInitials(name)}</Text>
        </View>
    );
};
...
const ImageGrid = ({ images, onImagePress }) => {
        ...
        return (
            <View style={s.imgRow}>
                {images.slice(0, 2).map((f, i) => (
                    <TouchableOpacity key={i} style={s.img2Wrap} onPress={() => onImagePress(i)} activeOpacity={0.9}>
    ...
    if (count === 3) {
        return (
            <View style={s.imgRow}>
                <TouchableOpacity style={s.img3Left} onPress={() => onImagePress(0)} activeOpacity={0.9}>
                    <AuthImage filename={images[0]} style={s.imgFill} />
                </TouchableOpacity>
                <View style={s.img3Right}>
                    {images.slice(1, 3).map((f, i) => (
                        <TouchableOpacity key={i} style={s.img3RightCell} onPress={() => onImagePress(i + 1)} activeOpacity={0.9}>
                            <AuthImage filename={f} style={s.imgFill} />
                        </TouchableOpacity>
                    ))}
                </View>
    ...
    return (
        <View style={s.imgGrid2x2}>
            {images.slice(0, 4).map((f, i) => (
                <TouchableOpacity key={i} style={s.img4Cell} onPress={() => onImagePress(i)} activeOpacity={0.9}>
                    <AuthImage filename={f} style={s.imgFill} />
                    {i === 3 && more > 0 && (
...
const ReactionButton = ({ myReaction, onReact }) => {
    ...
    return (
        <>
            <Pressable ref={btnRef} collapsable={false} style={s.actionBtn}
                onPress={handlePress} onLongPress={openPicker} delayLongPress={400}>
            ...
            <Modal visible={pickerVisible} transparent statusBarTranslucent animationType="none"
                onRequestClose={() => closePicker()}>
                <Pressable style={StyleSheet.absoluteFill} onPress={() => closePicker()}>
                    <Animated.View style={[s.pickerStrip, { top: pickerTop }, {
                        opacity: pickerAnim,
                        transform: [{ scale: pickerAnim.interpolate({ inputRange: [0, 1], outputRange: [0.6, 1] }) }],
                    }]}>
                        {REACTIONS.map((r) => (
                            <Pressable key={r.type} onPress={() => closePicker(() => onReact(r.type))} style={s.pickerItemCircle}>
                                <Text style={s.pickerEmojiLarge}>{r.emoji}</Text>
                                {myReaction?.type === r.type && <View style={s.pickerActiveDot} />}
                            </Pressable>
                        ))}
                    </Animated.View>
                </Pressable>
            ...
            <View style={s.rmOverlay}>
                <View style={s.rmSheet}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}
                        style={s.rmTabs} contentContainerStyle={{ gap: 6 }}>
                        <TouchableOpacity onPress={() => setFilter('all')}
                            style={[s.rmTab, filter === 'all' && s.rmTabActive]}>
                            <Text style={[s.rmTabText, filter === 'all' && s.rmTabTextActive]}>
                        ...
                        {types.map((type) => {
                            ...
                            return (
                                <TouchableOpacity key={type} onPress={() => setFilter(type)}
                                    ...
                                    <Text style={[s.rmTabText, filter === type && s.rmTabTextActive]}>
                                        {r.emoji}  {count}
                                    </Text>
                                </TouchableOpacity>
                            );
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <View style={s.rmRow}>
                                <AuthAvatar filename={item.author_avatar} name={item.author_name ?? '?'} size={36} />
                        ...
                        style={{ maxHeight: 340 }}
                        ListEmptyComponent={<Text style={s.rmEmpty}>Chưa có cảm xúc nào</Text>}
                    />
                </View>
            </View>
        </Modal>
...
// ── PostCard ──────────────────────────────────────────────────────────────────
const PostCard = ({ post, currentUser, onReact, onDelete, onPin, onCommentPress, canManage, onAuthorPress }) => {
    ...
    return (
        <View style={s.card}>
            ...
            <View style={s.authorRow}>
                <TouchableOpacity onPress={() => onAuthorPress?.(post.author_id)} activeOpacity={0.7}>
            ...
            {(totalReactions > 0 || legacyLikes?.length > 0 || post.comments_count > 0) && (
                <View style={s.statsRow}>
                    {totalReactions > 0 ? (
                        <TouchableOpacity style={s.statsItem} onPress={() => setReactionsOpen(true)} activeOpacity={0.7}>
            ...
            <View style={s.actionsRow}>
                ...
                <TouchableOpacity style={s.actionBtn} onPress={() => onCommentPress(post)}>
                ...
                <View style={s.previewWrap}>
                    {post.comments_count > 2 && (
                        <TouchableOpacity onPress={() => onCommentPress(post)} activeOpacity={0.7}>
                            <Text style={s.viewAllComments}>Xem tất cả {post.comments_count} bình luận</Text>
                        </TouchableOpacity>
                    )}
            ...
            <Modal visible={lightboxIdx !== null} transparent animationType="fade"
                ...
                <Pressable style={s.lightboxBg} onPress={() => setLightboxIdx(null)}>
                    {lightboxIdx !== null && post.images?.[lightboxIdx] && (
                        <AuthImage filename={post.images[lightboxIdx]} style={s.lightboxImg} resizeMode="contain" />
                    )}
                </Pressable>
            </Modal>

            <ReactionsModal visible={reactionsOpen} onClose={() => setReactionsOpen(false)} reactions={post.reactions ?? []} />
        </View>
    );
};
...
const s = StyleSheet.create({
    ...
    pickerActiveDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: BRAND, marginTop: 3 },
